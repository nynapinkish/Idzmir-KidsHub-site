...
document.addEventListener("DOMContentLoaded", async () => {
  console.log('========================================');
  console.log('🥕 CARROT SORTING GAME - NO ALERT VERSION');
  console.log('========================================');
  ...
  // Initialize game session for CARROT game
  console.log('\n🥕 Initializing carrot game...');
  const gameStarted = await initializeGame('Relational Concepts', 'same / different / carrot', 4);
  ...
  if (!gameStarted) {
    ...
    console.log('✅ Previous score displayed without alert');
    return;
  }
  ...
  dropZone.addEventListener("drop", async (e) => {
    ...
    const isCarrot = fruitType.toLowerCase().includes("carrot");

    totalAttempts++;
    console.log(`\n📊 DROP #${totalAttempts}/${maxAttempts}`);
    console.log('   Fruit:', fruitType);
    console.log('   Is Carrot?', isCarrot);
    ...
    if (isCarrot) {
      ...
      if (typeof handleCorrectAnswer === 'function') {
        ...
        console.error('   ❌ handleCorrectAnswer not found!');
      }
  ...
  console.log('\n✅ Carrot Sorting Game fully loaded!');
  console.log('========================================\n');
});